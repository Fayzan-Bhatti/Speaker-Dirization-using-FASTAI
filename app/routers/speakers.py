...
@router.post("/speakers_identification")
async def transcript_with_speakers(speakers : Speakers ,x_async_request: str = Header(None)):
    # In case if x-async-request header is true  (HTTP 202 Accepted)
    if x_async_request == "true":
        async_transcribe_internal(speakers.ext_ref_no, speakers.title, speakers.url,speakers.language)
        return {
            "id": 123,
            "ext_ref_no": speakers.ext_ref_no,
            "url": speakers.url,
        ...
        response = await transcribe_internal(speakers.ext_ref_no, speakers.title, speakers.url,speakers.language)
        return response
...
async def async_transcribe_internal(ext_ref_no, title, url, language):
    ...
    req = requests.post('your  URL where you want to save this data',
                        json={"ext_ref_no":ext_ref_no,
                        "title":title,
                        "url":url, 
                        "language":language},
                        )
...
async def transcribe_internal(ext_ref_no, title, url, language):
    ...
    if url.endswith('.mp4'):
        ...
        pipeline = Pipeline.from_pretrained('pyannote/speaker-diarization', use_auth_token=os.getenv("PYANOTATE_AUTH_TOKEN"))
        print(pipeline)
        ...
        with open(f"{ext_ref_no + '_' + 't.txt'}", "wb") as outfile:
            for i in range(gidx+1):
                subprocess.run(["whisper", ext_ref_no + '_' + str(i) + '.wav',"--language","en", "--model", "tiny"])
